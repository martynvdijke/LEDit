...
class Render(models.Model):
    ...
    def get_image(self):
        """
        Returns a dictionary containing image data and format
        """
        image_bytes = self.render()
        return {"format": "PNG", "data": base64.b64encode(image_bytes)}
    ...
    def render_dict(self, data_dict, image_size=(400, 400), theme=DEFAULT_THEME):
        ...
        for i in range(0, image_size[0], pixel_size):
            # Top border
            draw.rectangle(
                [i, 0, i + pixel_size - 1, pixel_size - 1],
            ...
            # Bottom border
            draw.rectangle(
                [i, image_size[1] - pixel_size, i + pixel_size - 1, image_size[1]],
                fill=theme.accent_color
                if i % (pixel_size * 2) != 0
                else theme.text_color,
            )
        ...
        for i in range(0, image_size[1], pixel_size):
            # Left border
            draw.rectangle(
                [0, i, pixel_size - 1, i + pixel_size - 1],
                fill=theme.accent_color
                if i % (pixel_size * 2) == 0
                else theme.text_color,
            )
            # Right border
            draw.rectangle(
                [image_size[0] - pixel_size, i, image_size[0], i + pixel_size - 1],
                fill=theme.accent_color
                if i % (pixel_size * 2) != 0
                else theme.text_color,
            )
        ...
        # Draw separator line
        draw.line(
            [(margin, y_position), (image_size[0] - margin, y_position)],
        ...
        for key, value in data_dict.items():
            # Draw pixel marker
            draw.rectangle(
                [margin - 15, y_position + 8, margin - 7, y_position + 16],
                fill=theme.accent_color,
            )
        ...
        for y in range(0, image_size[1], 4):
            draw.line([(0, y), (image_size[0], y)], fill=(0, 0, 0, 50), width=1)

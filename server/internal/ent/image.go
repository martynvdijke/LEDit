...
func (*Image) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case image.ForeignKeys[0]: // general_settings_images
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case image.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field general_settings_images", value)
			} else if value.Valid {
				_m.general_settings_images = new(int)
				*_m.general_settings_images = int(value.Int64)
			}
		default:

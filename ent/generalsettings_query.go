...
// QueryPixelArts chains the current query on the "pixel_arts" edge.
func (_q *GeneralSettingsQuery) QueryPixelArts() *PixelArtQuery {
	query := (&PixelArtClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *GeneralSettingsQuery) QueryPlaylists() *PlaylistQuery {
...
// WithPixelArts tells the query-builder to eager-load the nodes that are connected to
// the "pixel_arts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GeneralSettingsQuery) WithPixelArts(opts ...func(*PixelArtQuery)) *GeneralSettingsQuery {
	query := (&PixelArtClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPixelArts = query
	return _q
}
...
func (_q *GeneralSettingsQuery) WithPlaylists(opts ...func(*PlaylistQuery)) *GeneralSettingsQuery {
	query := (&PlaylistClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GeneralSettingsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*GeneralSettings, error) {
	...
	if query := _q.withPixelArts; query != nil {
		if err := _q.loadPixelArts(ctx, query, nodes,
			func(n *GeneralSettings) { n.Edges.PixelArts = []*PixelArt{} },
			func(n *GeneralSettings, e *PixelArt) { n.Edges.PixelArts = append(n.Edges.PixelArts, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GeneralSettingsQuery) loadPixelArts(ctx context.Context, query *PixelArtQuery, nodes []*GeneralSettings, init func(*GeneralSettings), assign func(*GeneralSettings, *PixelArt)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*GeneralSettings)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.PixelArt(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(generalsettings.PixelArtsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.general_settings_pixel_arts
		if fk == nil {
			return fmt.Errorf(`foreign-key "general_settings_pixel_arts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "general_settings_pixel_arts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *GeneralSettingsQuery) loadPlaylists(ctx context.Context, query *PlaylistQuery, nodes []*GeneralSettings, init func(*GeneralSettings), assign func(*GeneralSettings, *Playlist)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*GeneralSettings)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Playlist(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(generalsettings.PlaylistsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.general_settings_playlists
		if fk == nil {
			return fmt.Errorf(`foreign-key "general_settings_playlists" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {

...
type ThemeQuery struct {
	...
	withAssignments *ThemeAssignmentQuery
...
func (_q *ThemeQuery) QueryAssignments() *ThemeAssignmentQuery {
	query := (&ThemeAssignmentClient{config: _q.config}).Query()
...
func (_q *ThemeQuery) WithAssignments(opts ...func(*ThemeAssignmentQuery)) *ThemeQuery {
	query := (&ThemeAssignmentClient{config: _q.config}).Query()
...
func (_q *ThemeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Theme, error) {
	...
	if query := _q.withAssignments; query != nil {
		if err := _q.loadAssignments(ctx, query, nodes,
			func(n *Theme) { n.Edges.Assignments = []*ThemeAssignment{} },
			func(n *Theme, e *ThemeAssignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ThemeQuery) loadAssignments(ctx context.Context, query *ThemeAssignmentQuery, nodes []*Theme, init func(*Theme), assign func(*Theme, *ThemeAssignment)) error {
	...
	query.withFKs = true
	query.Where(predicate.ThemeAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(theme.AssignmentsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.theme_assignments
		if fk == nil {
			return fmt.Errorf(`foreign-key "theme_assignments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "theme_assignments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
